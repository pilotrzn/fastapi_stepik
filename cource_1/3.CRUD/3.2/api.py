...
async def create_message(message: MessageCreate) -> Message:
    ...
    new_message = Message(id=next_id, content=message.content)
    messages_db.append(new_message)
    return new_message


@app.put("/messages/{message_id}", response_model=Message)
async def update_message(message_id: int, create_message: MessageCreate) -> Message:
    for i, message in enumerate(messages_db):
        if message.id == message_id:
            updated_message = Message(id=message_id, content=create_message.content)
